function [lfv,vsigma2_rc,xitt,xit1t]=lfv1c_ms(thtr)
...
k_1=exp(thtr(3));
k_2=exp(thtr(4));
delta1=exp(thtr(5))/(1+exp(thtr(5))+exp(thtr(6)));
alpha1=exp(thtr(6))/(1+exp(thtr(5))+exp(thtr(6)));
p11=exp(thtr(7))/(1+exp(thtr(7)));
p22=exp(thtr(8))/(1+exp(thtr(8)));
%
P=[p11    1-p22;
...
for t=1:T
    ...
    sigma21_2=k_1+delta1*sigma20+alpha1*e20;
    ...
    xit1t(t,:)=xitt(t,:)*P';    %esto es como  P*xitt(t,:)'     la predicción a 1 periodo de eta?
    %
    p10=xit1t(t,1);
